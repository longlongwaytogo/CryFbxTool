...
CPlayer::~CPlayer()
{
	...
	gEnv->pInput->RemoveEventListener(m_pInputEvent);
	gEnv->pSystem->UnregisterWindowMessageHandler(m_pInputEvent);
}
...
void CPlayer::PostInit(IGameObject *pGameObject)
{
	...
	m_pInputEvent = static_cast<CInputEvent*>(GetGameObject()->AcquireExtension("InputEvent"));
	gEnv->pInput->AddEventListener(m_pInputEvent);

	gEnv->pSystem->RegisterWindowMessageHandler(m_pInputEvent);
