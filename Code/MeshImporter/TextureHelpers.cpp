...
{	
	...
	{
		return string(QtUtil::ToString(QDir(absSourceOriginalDirectoryPath).relativeFilePath(absSourceTexturePath)).c_str());
	}
	else if (absSourceTexturePath.startsWith(absSourceDirectoryPath))
	{
		...
		return string(QtUtil::ToString(QDir(absSourceDirectoryPath).relativeFilePath(absSourceTexturePath)).c_str());
	}
	else
	{
