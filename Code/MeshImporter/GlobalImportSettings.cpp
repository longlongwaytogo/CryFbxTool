...
//#include <Material\IMaterialManager.h>
...
static std::string FormatUnitSizeInCm(double scale)
{
	for (int i = 0; i < FbxMetaData::Units::eUnitSetting_COUNT; ++i)
	{
		...
		if (FbxMetaData::Units::eUnitSetting_FromFile != unit)
		{
			...
			{
				return std::string(FbxMetaData::Units::GetSymbol(unit));
			}
		}
	}
...
void CGlobalImportSettings::SConversionSettings::Serialize(yasli::Archive& ar)
{
	...
	options[0] += string(" (") + string(fileUnitSizeInCm.c_str()) + string(")");
